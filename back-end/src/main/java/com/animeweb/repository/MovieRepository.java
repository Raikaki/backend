...
public interface MovieRepository extends JpaRepository<Movie,Long> {
    ...
    @Query("SELECT DISTINCT m FROM Movie m JOIN View v ON m.id = v.movie.id WHERE DATE(v.watchAt) = CURDATE() AND m.status = true ORDER BY SIZE(m.views) DESC")
    List<Movie> findTopMoviesByDate();
    @Query("SELECT DISTINCT m FROM Movie m JOIN View v ON m.id = v.movie.id   WHERE YEAR(v.watchAt) = YEAR(CURDATE()) AND MONTH(v.watchAt) = MONTH(CURDATE()) AND m.status = true ORDER BY SIZE(m.views) DESC")
    List<Movie> findTopMoviesMonth();
    @Query("SELECT DISTINCT m FROM Movie m JOIN View v ON m.id = v.movie.id  WHERE YEAR(v.watchAt) = YEAR(CURDATE()) AND m.status = true ORDER BY SIZE(m.views) DESC")
